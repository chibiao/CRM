...
function daochu() {
    location.href="/service/daochu";
}
function update(id) {
    location.href="/service/getById?id="+id;
}
function select(id) {
    location.href="/service/getByIdGui?id="+id;
}

function fenye(curPage) {
	var custom=document.getElementById("custom").value;
	location.href="/service/listGuidang?curPage="+curPage+"&custom="+custom;
}
function query(){
	   var custom=document.getElementById("custom").value;
	   ...
	   //alert(types)
		location.href="/service/listGuidang?curPage=1&custom="+custom+"&id="+id+"&types="+types;
	}
</script>
</head>
<body>
	<input type="button" value="帮助" class="anNiu"/>
	<input type="button" value="导出excel" class="anNiu" onclick="daochu()"/>
	...
	<table class="tab_bottom">
		...
		<tr>
				...
				<td onclick="select('${cj.id}')"><img src="/static/images/service/sel.png" /></td>
		</tr>
		</c:forEach>
		<tr>
            <td colspan="7">
