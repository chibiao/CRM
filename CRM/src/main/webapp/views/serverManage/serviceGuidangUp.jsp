...
function fanhui() {
    location.href="/service/listGuidang";
}
function baocun(id) {
...
<form action="/service/save" method="post">
	...
	<table class="tab_bottom">
		...
		<tr>
			...
			<td>
				${sercj.serresult.result}
			</td>
			<td><font class="biaoTi">满意度</font></td>
			<td>
				<c:if test="${sercj.serresult.satisfied==1}">
					★
				</c:if>
				<c:if test="${sercj.serresult.satisfied==2}">
					★★
				</c:if>
				<c:if test="${sercj.serresult.satisfied==3}">
					★★★
				</c:if>
				<c:if test="${sercj.serresult.satisfied==4}">
					★★★★
				</c:if>
				<c:if test="${sercj.serresult.satisfied==5}">
					★★★★★★
				</c:if>
			</td>
		</tr>
