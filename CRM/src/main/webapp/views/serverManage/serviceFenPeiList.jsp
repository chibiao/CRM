...
function update(id) {
    location.href="/sercj/getChuLiById?id="+id;
}
function del(id) {
if(confirm("你确认删除吗?"))
...
location.href="/sercj/fplist?curPage="+curPage+"&custom="+custom;
}
function query(){
	   var custom=document.getElementById("custom").value;
	   ...
	   var types=t.options[index].value;
		location.href="/sercj/fplist?curPage=1&custom="+custom+"&id="+id+"&types="+types;
	}
</script>
<body>
	...
	<table class="tab_bottom">
		<tr>
			...
			<td><font class="biaoTi">分配人</font></td>
			<td><font class="biaoTi">分配日期</font></td>
		...
		<tr>
				...
				<td>${cj.serfp.cname}</td>
				<td>${cj.serfp.ftime}</td>
